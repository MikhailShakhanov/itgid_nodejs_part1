...
async function f4() {
    ...
    conn.end();
    return [rows1[0].coin, rows2[0].email];
}
...
// Порядок - по id (возрастание). Т.е. я ожидаю от вас строку 4 8 12 56 43. Задача выполняется выборкой и последующей обработкой массива.

async function f5() {
    const  conn = await mysql.createConnection(config);
    const [ rows] = await conn.execute(`select coin from user order by id asc`);
    conn.end();    
    return rows.reduce( (ac, cur) => ac + (ac ? ' ': '') + cur.coin, '');
}
...
// а вторым запросом записывает это firstname для пользователя с lastname Bro. Также функция должна возвращать данное firstname.
